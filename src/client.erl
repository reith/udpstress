...
child_spec(Addr, Port) ->
  ID = list_to_atom(lists:flatten(io_lib:format("client_~p_~p", [inet:ntoa(Addr), Port]))),
  #{
     id => ID,
     start => {?MODULE, start_link, [ID, #{port => Port, addr => Addr}]},
     restart => permanent,
     modules => [?MODULE],
     type => worker
  }.
